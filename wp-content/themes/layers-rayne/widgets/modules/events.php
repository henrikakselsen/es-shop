 ...
 * @since Layers 1.0.0
 */
function layers_school_widgets_init() {

	if( !class_exists( 'Layers_Content_Events_Widget' ) ) {
		class Layers_Content_Events_Widget extends Layers_Widget {

			/**
			*  Widget construction
			*/
			function Layers_Content_Events_Widget(){
				...
				*/
				$this->widget_title = __( 'Events' , 'layerswp' );
				$this->widget_id = 'events';
				$this->post_type = '';
				$this->taxonomy = '';
				...
				$this->defaults = array (
					'title' => __( 'Upcoming Events', 'layerswp' ),
					'excerpt' => __( 'We have some exciting events coming in soon, check them out below!', 'layerswp' ),
					'design' => array(
						'layout' => 'layout-boxed',
						'liststyle' => 'list-grid',
						'columns' => '3',
				...
				$this->column_defaults = array (
					'title' => __( 'Your event title', 'layerswp' ),
					'excerpt' => __( 'Give us a brief description of the event that you are promoting. Try keep it short so that it is easy for people to scan your page.', 'layerswp' ),
					'width' => '4',
					'design' => array(
			...
			*/
			function widget( $args, $instance ) {
				...
				*/
				...
				$widget_container_class[] = 'widget row content-vertical-massive events';
								...
								<div id="<?php echo $widget_id; ?>-<?php echo $column_key; ?>" class="<?php echo $column_class; ?>">
									...
									*/
									...
									<div class="<?php echo $column_inner_class; ?>">
										...
										<?php if(
												$this->check_and_return( $column, 'title' )
												||
												$this->check_and_return( $column, 'excerpt' )
												||
												$this->check_and_return( $column, 'link_text' )
												||
												$this->check_and_return( $column, 'date' )
												||
												$this->check_and_return( $column, 'time' )
												||
												$this->check_and_return( $column, 'place' )
												||
												$this->check_and_return( $column, 'contact' )
											) { ?>
											<div class="media-body <?php echo ( isset( $column['design']['fonts'][ 'align' ] ) ) ? $column['design']['fonts'][ 'align' ] : ''; ?>">
												<?php if( $this->check_and_return( $column, 'title') ) { ?>
													<h5 class="heading">
														<?php if( NULL != $link && ! ( isset( $column['link'] ) && $this->check_and_return( $column , 'link_text' ) ) ) { ?><a href="<?php echo $column['link']; ?>"><?php } ?>
															<?php echo esc_attr( $column['title'] ); ?>
												...
												<?php if( $this->check_and_return( $column, 'date' ) || $this->check_and_return( $column, 'time' ) || $this->check_and_return( $column, 'place' ) || $this->check_and_return( $column, 'contact' ) ) {
													$meta = array(
														'date' => __( 'Date' , 'layers-woocommerce' ),
														'time' => __( 'Time' , 'layers-woocommerce' ),
														'place' => __( 'Place' , 'layers-woocommerce' ),
														'contact' => __( 'Contact' , 'layers-woocommerce' ),
													); ?>
													<ul class="event-meta">
														<?php foreach( $meta as $meta_key => $meta_label ) { ?>
															<?php if ( $this->check_and_return( $column, $meta_key ) ) { ?>
																<li class="event-meta-row">
																	<?php if ( $this->check_and_return( $column, $meta_key ) ) { ?>

																		<span class="meta-type">
																			<?php echo esc_attr( $meta_label ); ?>:
																		</span>
																		<span class="meta-data">
																			<?php if( FALSE === strpos( $this->check_and_return( $column, $meta_key ), 'http' ) ) { ?>
																				<?php echo esc_attr( $this->check_and_return( $column, $meta_key ) ); ?>
																			<?php } else { ?>
																				<a href="<?php echo esc_attr( $this->check_and_return( $column, $meta_key ) ); ?>" target="_blank">
																					<?php echo  esc_attr( $this->check_and_return( $column, $meta_key ) ); ?>
																				</a>
																			<?php } ?>
												...
												<?php if( isset( $column['link'] ) && $this->check_and_return( $column , 'link_text' ) ) { ?>
													<a href="<?php echo  esc_attr( $column['link'] ); ?>" class="button btn-<?php echo $this->check_and_return( $column , 'design' , 'fonts' , 'size' ); ?>"><?php echo  esc_attr( $column['link_text'] ); ?></a>
												<?php } ?>
											</div>
										<?php } ?>
									</div>
					...
					<section class="layers-accordion-section layers-content">
						...
						<?php echo $this->form_elements()->input(
							array(
								...
								'id' => 'events_column_ids_input_' . $this->number,
						...
						<ul id="events_column_list_<?php echo esc_attr( $this->number ); ?>" class="layers-accordions layers-accordions-sortable layers-sortable" data-id_base="<?php echo $this->id_base; ?>" data-number="<?php echo $this->number; ?>">
						...
						<button class="layers-button btn-full layers-add-widget-events add-new-widget" data-number="<?php echo esc_attr( $this->number ); ?>"><?php _e( 'Add New Column' , 'layerswp' ) ; ?></button>
					</section>
				</div>
						...
						<section class="layers-accordion-section layers-content">
							...
							<div class="layers-row layers-push-bottom-small">
								<p class="layers-form-item layers-column layers-span-6">
									<label for="<?php echo $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'date' ); ?>">
										<?php _e( 'Date' , 'layerswp' ); ?>
									</label>
									<?php echo $this->form_elements()->input(
										array(
											'type' => 'text',
											'name' => $this->get_custom_field_name( $widget_details, 'columns',  $column_guid, 'date' ),
											'id' => $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'date' ),
											'placeholder' => __( '07 September 2015' , 'layerswp' ),
											'value' => ( isset( $column['date'] ) ) ? $column['date'] : NULL ,
											'class' => 'layers-text',
										)
									); ?>
								</p>
								<p class="layers-form-item layers-column layers-span-6">
									<label for="<?php echo $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'time' ); ?>">
										<?php _e( 'Time' , 'layerswp' ); ?>
									</label>
									<?php echo $this->form_elements()->input(
										array(
											'type' => 'text',
											'name' => $this->get_custom_field_name( $widget_details, 'columns',  $column_guid, 'time' ),
											'id' => $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'time' ),
											'placeholder' => __( '14:30' , 'layerswp' ),
											'value' => ( isset( $column['time'] ) ) ? $column['time'] : NULL ,
										)
									); ?>
								</p>
							</div>
							...
							<div class="layers-row layers-push-bottom-small">
								<p class="layers-form-item layers-column layers-span-6">
									<label for="<?php echo $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'place' ); ?>">
										<?php _e( 'Place' , 'layerswp' ); ?>
									</label>
									<?php echo $this->form_elements()->input(
										array(
											'type' => 'text',
											'name' => $this->get_custom_field_name( $widget_details, 'columns',  $column_guid, 'place' ),
											'id' => $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'place' ),
											'placeholder' => __( 'Green Point, Cape Town' , 'layerswp' ),
											'value' => ( isset( $column['place'] ) ) ? $column['place'] : NULL ,
											'class' => 'layers-text',
										)
									); ?>
								</p>
								<p class="layers-form-item layers-column layers-span-6">
									<label for="<?php echo $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'contact' ); ?>">
										<?php _e( 'Contact' , 'layerswp' ); ?>
									</label>
									<?php echo $this->form_elements()->input(
										array(
											'type' => 'text',
											'name' => $this->get_custom_field_name( $widget_details, 'columns',  $column_guid, 'contact' ),
											'id' => $this->get_custom_field_id( $widget_details, 'columns',  $column_guid, 'contact' ),
											'placeholder' => __( '086 555 1234' , 'layerswp' ),
											'value' => ( isset( $column['contact'] ) ) ? $column['contact'] : NULL ,
										)
									); ?>
								</p>
							</div>
		...
		register_widget("Layers_Content_Events_Widget");
	}

}

add_action( 'widgets_init' , 'layers_school_widgets_init' , 40 );